...
import { FiHome, FiMusic, FiUpload, FiUser, FiLogOut, FiMenu,
  FiX, FiSearch, FiSettings } from 'react-icons/fi';
...
const Navbar = () => {
  ...
  const navItems = [
    { to: '/dashboard', icon: <FiHome size={20} />, label: 'Dashboard', show: true },
    { to: '/snippets', icon: <FiMusic size={20} />, label: `${user?.role ==='artist' ? 'My Snippets':'All Snippets'}`, show: true },
    {
      to: '/upload-snippet',
    ...
    { to: '/profile', icon: <FiUser size={20} />, label: 'Profile', show: true },
    { to: '/settings', icon: <FiSettings size={20} />, label: 'Settings', show: true }
  ];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (window.innerWidth <= 768 && !event.target.closest('.navbar-container')) {
        setIsCollapsed(true);
      }
    };
    ...
    >
      ...
      <div className="flex justify-between p-4 items-center gap-5 relative">
        <NavLink to="/dashboard" className="flex items-center">
          {isCollapsed ? (''
            // <img src={logoIcon} alt="HypeDrop" className="h-8 w-8" />
          ) : (''
        ...
        <button
          ...
          className={`absolute p-1 rounded-full hover:bg-gray-100 transition-colors ${
            isCollapsed ? '-right-4' : 'right-0'
          }`}
      ...
      <div className="p-4 border-t border-gray-100">
        {!isCollapsed && user && (
          <div className="flex items-center mb-4 p-2 rounded-lg hover:bg-gray-50 transition-colors">
            <div className="w-8 h-8 rounded-full bg-[#7D00FF]/10 flex items-center justify-center text-[#7D00FF] font-medium">
              {user?.role === 'artist' ? user?.stageName?.charAt(0): user?.userName?.charAt(0)}
            </div>
        ...
        <button
          ...
          className="flex cursor-pointer items-center gap-3 text-gray-700 hover:text-[#7D00FF] transition-colors w-full p-2 rounded-lg hover:bg-gray-50"
          aria-label="Logout"
        >
          <FiLogOut size={20} />
